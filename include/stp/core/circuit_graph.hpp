...
{
public:
  Gate(Type type, line_idx output, std::vector<line_idx> &&inputs)
  ...
  const line_idx &get_output() const
  {
    return m_output;
  }
  line_idx &output()
...
public:
  ...
  const Gate &get_gate(const gate_idx &idx) const
  {
    return m_gates[idx];
  }
  Gate &gate(const gate_idx &idx)
  {
    return m_gates[idx];
  }

  const Line &get_line(const line_idx &idx) const
  {
    return m_lines[idx];
  }
  Line &line(const line_idx &idx)
  {
    return m_lines[idx];
  }
  std::vector<line_idx> &inputs()
